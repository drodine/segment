...
ifdef BUILD_NUMBER
	...
	APP_TEST_RESULTS_FILE := $(TST_DIR)/$(APPNAME)_$(MAJOR_VERSION).$(MINOR_VERSION).$(BUILD_NUMBER).log
else
	APP_ZIP_FILE := $(OUT_DIR)/$(APPNAME).zip
	APP_PKG_FILE := $(PKG_DIR)/$(APPNAME).pkg
	APP_TEST_RESULTS_FILE := $(TST_DIR)/$(APPNAME).log
endif
...
APP_INCLUDES = source resources manifest components
...
test: testbuild zip cleanup remove deploy runtests
...
testbuild:
	...
	cp -r $(SEGMENT_LIB_DIR)/components $(BUILDDIR)/components
	cp -r $(SEGMENT_LIB_DIR)/source $(BUILDDIR)/source
	cp ./test_manifest $(BUILDDIR)/manifest
...
build:
	...
	@if [ ! -z $(COUNTRY_CODE) ]; \
	then \
		sed -e s/"\' countryCode = \"US\""/"countryCode = \"$(COUNTRY_CODE)\""/ ./components/tasks/CrackleAPITask.brs > $(BUILDDIR)/components/tasks/CrackleApiTask.brs; \
	fi

	@if [ ! $(BUILD_TYPE) = "prod" ]; \
	then \
		sed -e 's/"\[BRANCH_NAME\]"/"$(BRANCH)"/;s/"\[BUILD_TYPE\]"/"$(BUILD_TYPE)"/' ./components/views/SettingsCellView.brs > ./build/components/views/SettingsCellView.brs; \
		sed -e s/"\' m.settingsItems.push({cellType:\"devinfo\"})"/"m.settingsItems.push({cellType:\"devinfo\"})"/ ./components/screens/MyCrackleScreen.brs > $(BUILDDIR)/components/screens/MyCrackleScreen.brs; \
	fi

	@if [ $(BUILD_TYPE) = "stage" ]; \
	then \
		sed -e "s/http\:\/\/2517d\.v\.fwmrm\.net\/ad\/g\/1\?nw=151933\&prof=151933:crackle_prime_ctv_live/http:\/\/23adc\.v\.fwmrm.net\/ad\/g\/1\?nw=146140\&prof=146140:crackle_prime_ctv_test/" ./resources/config.json > ./build/resources/config.json; \
		sed -e "s/http\:\/\/web-us-az.crackle.com/http\:\/\/cracklestaging.blob.core.windows.net\/cdnen-us/;s/api-v1-us/staging-v1-api-us/" ./resources/config.json > $(BUILDDIR)/resources/config.json; \
	fi

	@if [ $(BUILD_TYPE) = "test" ]; \
	then \
		sed -e "s/cfg.printHb = false/cfg.printHb = true/;s/cfg.enableLogging = false/cfg.enableLogging = true/" ./source/libs/Conviva_Roku_SgClient.brs > $(BUILDDIR)/source/libs/Conviva_Roku_SgClient.brs; \
		sed -e "s/\"touchstone\": \"\"/\"touchstone\": \"https:\/\/sony-test.testonly.conviva.com\"/" ./resources/config.json > $(BUILDDIR)/resources/config.json; \
		sed -e s/"\' return true \' Test Evn"/"return true \' Test Evn"/ ./source/constants.brs > ./build/source/constants.brs; \
	fi
...
zip:
	...
	pushd ./$(BUILDDIR)/; \
	zip -q -9 -r "../$(APP_ZIP_FILE)" . -i \*.png $(ZIP_EXCLUDES); \
...
addtests:
	...
	cp -r $(TEST_LIBRARIES) $(BUILDDIR)/source 2>/dev/null || :
...
runtests:
	...
	telnet $(ROKU_DEV_TARGET) 8085 | tee -a $(APP_TEST_RESULTS_FILE)
